...
load('SS1.RData')
SS1 <- FindVariableFeatures(SS1, nfeatures = 5000)
E0 <- SS1@reductions$umap@cell.embeddings
mdX <- SS1@meta.data
...
D6[[1]] <- D6[[1]][VariableFeatures(SS1),]
D6[[2]] <- D6[[2]][VariableFeatures(SS1),]
...
D8[[1]] <- D8[[1]][VariableFeatures(SS1),]
D8[[2]] <- D8[[2]][VariableFeatures(SS1),]
...
E <- SS1@reductions$umap@cell.embeddings
rownames(E) <- rownames(mdX)
S <- S[,rownames(mdX)]
U <- U[,rownames(mdX)]
D <- SS1@reductions$pca@cell.embeddings
...
save(velocity, arrows , file = 'scVelocity_SS1.RData')