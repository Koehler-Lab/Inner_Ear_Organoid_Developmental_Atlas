...
# Loading SS1
load('/lab-share/RC-Data-Science-e2/Public/Daniel/p147-scRNAseq-Koehler/SS1.RData')
X <- X[,colnames(SS1)]
SS1 <- SS1[,colnames(X)]

# Velocity
V <- doVelocity(X, 'stochastic')
VF <- getVectorField(SS1,V, reduction = 'umap', resolution = 50)

save(VF, file = 'scVelo_SS1.RData')
